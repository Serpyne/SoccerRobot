...
# Instructions:
1. <h6>connect the ev3 brick via bluetooth</h6><br>
	scan for devices and connect to the laptop
	make sure it says ***status: connected***
2. <h6>open command prompt **(cmd)**</h6><br>
	type in `ipconfig /all`
	find the one that says **bluetooth connection**
	copy the **physical address**
3. <h6>paste this into the *options.json* file, as **host_address**.</h6><br>
	replace the **-** with **:** just like the one already in the file
	save file
4. <h6>run *server.py*, make sure that it says server listening on address:port.</h6><br>
	make sure **tkinter** is installed
	this can be done by entering `pip install tkinter` in terminal
5. <h6>once server is listening, run *ROBIN.py* on the ev3.</h6><br>
