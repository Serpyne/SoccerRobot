...
def turn_left():
    motors[0].run(-TURN_SPEED)
    motors[1].run(-TURN_SPEED)
    motors[2].run(TURN_SPEED)
    motors[3].run(TURN_SPEED)
...
while True:
    if active:
        if facing_forward:
            ...
            if relative_ball_direction:
                ...
                if relative_ball_direction > 3*pi/2 or relative_ball_direction < pi/2:
                    ...
                    if relative_ball_direction > pi - .3 and relative_ball_direction < pi + .3:
                        move(3*pi/2)
                    else:
                        move(pi)
                print(round(relative_ball_direction, 3))
        ...
        if see_ball:
            orientation = (compass.value() - original_orientation) % 360
            if orientation > 360 - forward or orientation < forward:
                if not turned:
                    stop_motors()
                    facing_forward = True
                    turned = True
