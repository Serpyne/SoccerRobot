...
while True:
    if active:
        ...
        #     move(relative_ball_direction - pi/2)
            # relative_ball_direction = (relative_ball_direction - pi/2) % (2*pi) # forward is 0, backward is pi
            # orientation = (radians(compass.value()) - original_orientation) % (2*pi)
            # print(round(orientation, 3), compass.value())

            # global_ball_direction = (relative_ball_direction - orientation) % (2*pi)

            # # LOGIC
            # if global_ball_direction > 3*pi/2 or global_ball_direction < pi/2:
            #     if global_ball_direction > 2*pi-detection_extent or global_ball_direction < detection_extent:
            #         move(0 - orientation)
            #     else:
            #         if global_ball_direction < pi:
            #             move(pi/2 - orientation)
            #         else:
            #             move(3*pi/2 - orientation)

            # else:
            #     if global_ball_direction > pi - .3 and global_ball_direction < pi + .3:
            #         move(3*pi/2 - orientation)
            #     else:
            #         move(pi - orientation)
        # else:
