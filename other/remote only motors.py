    ...
    while True:
        if mode == MOTORS:
            ...
            if vel[0] == 0:
                ...
                motors[0].run_forever(speed_sp=abs(vel[0])*.6)
                motors[2].run_forever(speed_sp=abs(vel[0])*.6)
            elif vel[0] > 0:
                motors[0].polarity = "inversed"
                motors[2].polarity = "normal"
                motors[0].run_forever(speed_sp=vel[0]*.6)
                motors[2].run_forever(speed_sp=vel[0]*.6)
            if vel[1] == 0:
                motors[1].stop()
                motors[3].stop()
            elif vel[1] < 0:
            ...
            if request:
                ...
                if "change_movement_mode" in r:
                    ...
                    try:
                        ...
                        if not (v[0] == 0.0 and v[1] == 0.0):
                            a = atan2(v[1], v[0]) # compass correction
                            # a -= (compass_angle) * pi/180
                            vel = [round(cos(a)*1000, 1), round(sin(a)*1000, 1)]
